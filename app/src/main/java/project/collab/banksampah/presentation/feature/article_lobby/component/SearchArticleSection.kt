...
import project.collab.banksampah.presentation.feature.article_lobby.ArticleUiState
...
fun LazyListScope.searchArticleSection(
        ...
        onSuccess = {
            when {
                articles.itemCount == 0 -> {
                    item {
                        CommonEmptyState(
                            message = if (articlesUiState.searchQuery.isNotBlank()) {
                                "Tidak ada Artikel \"${articlesUiState.searchQuery}\""
                            } else {
